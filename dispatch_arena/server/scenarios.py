    ...
    for i in range(config.num_orders):
        ...
        orders.append(
            Order(
                ...
                created_tick=0,
                prep_remaining=prep,
                deadline_tick=min(config.max_ticks, deadline_base + extra_deadline),
                status=OrderStatus.READY if prep == 0 else OrderStatus.QUEUED,
            )
        )

    return Scenario(config=config, nodes=nodes, travel_time_matrix=travel, couriers=couriers, orders=orders)
